...
สคริปต์การติดตั้งจะทำการตรวจสอบหลายๆ อย่างเพื่อให้ระบบพร้อมใช้งาน รวมถึงการตรวจสอบสภาพแวดล้อม การติดตั้งไดรเวอร์ที่จำเป็น เช่น CUDA และการติดตั้ง Miniconda พร้อมกับไลบรารีและเครื่องมือที่ต้องการ

## คุณสมบัติ

- การติดตั้ง Miniconda อัตโนมัติ (หากยังไม่ได้ติดตั้ง)
...
3. **ข้อกำหนดด้านซอฟต์แวร์:**
   - Git
   - Miniconda (ถ้ายังไม่ได้ติดตั้ง สคริปต์จะช่วยติดตั้งให้)
   - Python 3.12
   - PIP 25.0
   - CUDA 12.8.1
   - cuDNN 9.8.0.87
...
3. **รันสคริปต์การติดตั้ง:**
   ...
   - ติดตั้ง Miniconda (ถ้ายังไม่ได้ติดตั้ง)
...
4. **การตั้งค่าตัวแปรสภาพแวดล้อม:**
   สคริปต์จะตั้งค่าตัวแปรสภาพแวดล้อมที่จำเป็น (Miniconda และ CUDA) ให้โดยอัตโนมัติ
...
- **ไม่พบ Conda Environment:**
  หากสคริปต์ไม่พบ Conda environment ให้ลบโฟลเดอร์ `conda_embeded` และ `env_facefusion` จากโฟลเดอร์ที่ติดตั้ง FaceFusion และรันสคริปต์ `auto_install_facefusion.bat` ใหม่ สคริปต์จะสร้าง environment ใหม่ให้โดยอัตโนมัติ ให้มั่นใจว่าได้ติดตั้ง Miniconda อย่างถูกต้อง
...
- **Python** 3.12
- **Miniconda** (สำหรับการจัดการ Python และ environment)
- **CUDA** 12.8.1
- **cuDNN** 9.8.0.87
- **PIP** 25.0
- **Git** (สำหรับการโคลน repository)
- **curl** (สำหรับดาวน์โหลด Miniconda)
